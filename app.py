...
try:
    ...
    with col1:
        ...
        st.dataframe(pd.DataFrame(ob['bids'], columns=['Price', 'Qty']).head(5))
    with col2:
        st.write("🔴 Asks (Sellers)")
        st.dataframe(pd.DataFrame(ob['asks'], columns=['Price', 'Qty']).head(5))

    time.sleep(2)
    st.rerun()
except Exception as e:
    st.error(f"Market data load ho raha hai... {e}")
    time.sleep(2)
    st.rerun()
